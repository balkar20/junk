...
public static class PowerShellHandler
{
    ...
    public static string Command(string script, Dictionary<string, string> customParams = null, IEnumerable<string>? customArgs = null)
    {
        ...
        // {
        //     _ps.AddParameters(customParams);
        // }

        var enumerable = customArgs?.ToList();
        if (enumerable != null && customArgs != null && enumerable.Any())
        {
            foreach (var customArg in enumerable)
            {
                _ps.AddArgument(customArg);
            }
        }
